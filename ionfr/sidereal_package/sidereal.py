...
def parseDate(s):
    ...
    m = DATE_PAT.match(s)
    if m is None:
        raise SyntaxError("Date does not have pattern YYYY-DD-MM: " "'%s'" % s)
    # -- 2 --
...
def parseTime(s):
    ...
    if minuteTail.startswith(":"):
        ...
        if m is None:
            raise SyntaxError("Expecting minutes: '%s'" % minuteTail)
    ...
    if secondTail.startswith(":"):
        ...
        if m is None:
            raise SyntaxError("Expecting seconds: '%s'" % secondTail)
...
def parseFixedZone(s):
    ...
    if s.startswith("+"):
        ...
        raise SyntaxError("Expecting zone modifier as %shhmm: " "'%s'" % (s[0], s))
    # -- 2 --
...
def parseAngle(s):
    ...
    #   else -> raise SyntaxError ]
    degree, minTail = parseFloatSuffix(s, D_PAT, "Degrees followed by 'd'")
    ...
    if len(minTail) != 0:
        ...
        #   else -> raise SyntaxError ]
        minute, secTail = parseFloatSuffix(minTail, M_PAT, "Minutes followed by 'm'")
        ...
        if len(secTail) != 0:
            ...
            if len(checkTail) != 0:
                raise SyntaxError("Unidentifiable angle parts: " "'%s'" % checkTail)
    # -- 4 --
...
def parseLat(s):
    ...
    m = NS_PAT.match(last)
    if m is None:
        raise SyntaxError("Latitude '%s' does not end with 'n' " "or 's'." % s)
    nsFlag = last.lower()
    # -- 3 --
...
def parseLon(s):
    ...
    m = EW_PAT.match(last)
    if m is None:
        raise SyntaxError("Longitude '%s' does not end with " "'e' or 'w'." % s)
    ewFlag = last.lower()
    # -- 3 --
    # [ if rawAngle is a valid angle ->
    #     absAngle  :=  that angle in radians
...
def parseHours(s):
    ...
    #     minTail  :=  s after that float and suffix
    #   else -> raise SyntaxError ]
    hour, minTail = parseFloatSuffix(s, H_PAT, "Hours followed by 'h'")
...
class MixedUnits:
    ...
    def __pad(self, coeffs):
        ...
        if shortage < 0:
            raise ValueError(
                "Value %s has too many elements; " "max is %d." % (coeffs, stdLen)
            )
        if shortage > 0:
            result += [0.0] * shortage
    ...
    def format(self, coeffs, decimals=0, lz=False):
        ...
        if lz:
            ...
            fmt = "%d"
        result = [fmt % x for x in coeffList[:-1]]
        # -- 2 --
...
class SiderealTime:
    ...
    def utc(self, date):
        ...
        #           interval [0,24) ]
        t0 = (nDays * SIDEREAL_A - SiderealTime.factorB(date.year)) % 24.0
        # -- 3 --
    ...
    def fromDatetime(dt):
        ...
        nDays = dayNo(utc)
        # -- 3 --
        t0 = nDays * SIDEREAL_A - SiderealTime.factorB(utc.year)
        # -- 4 --
        # [ decUTC  :=  utc as decimal hours ]
        floatSec = utc.second + float(utc.microsecond) / 1e6
        decUTC = dmsUnits.mixToSingle((utc.hour, utc.minute, floatSec))
        # -- 4 --
...
class AltAz:
    ...
    def __str__(self):
        ...
        #       seconds ]
        altList = dmsUnits.format(
            dmsUnits.singleToMix(degrees(self.alt)), lz=True, decimals=3
        )
        azList = dmsUnits.format(
            dmsUnits.singleToMix(degrees(self.az)), lz=True, decimals=3
        )

        # -- 2 --
        return "[az %sd %s' %s\" alt %sd %s' %s\"]" % (tuple(azList) + tuple(altList))
...
class RADec:
    ...
    def __str__(self):
        ...
        raUnits = dmsUnits.format(
            dmsUnits.singleToMix(radiansToHours(self.ra)), lz=True, decimals=3
        )
        decUnits = dmsUnits.format(
            dmsUnits.singleToMix(degrees(self.dec)), lz=True, decimals=3
        )

        # -- 2 --
        return "[%sh %sm %ss, %sd %s' %s\"]" % (tuple(raUnits) + tuple(decUnits))
