...
def calcRMSTEC(coordLat, coordLon, filename):
                ...
                for itemLat in range(int(pointsLat)):
                    if (
                        ...
                        for item in range(len(NewLongList[i + 7 + counterLat].split())):
                            ...
                            counterLon = counterLon + 1
                    if (
                        "-" + NewLongList[i + 2 + counterLat].split()[0].split("-")[1]
                        == "" + str(newstartLat) + ""
                    ):
                        # Adding to array 'a' a line of Latitude RMS TEC data
        ...
        for lat in range(int(pointsLat)):
            for lon in range(int(pointsLon)):
                ...
                if (lon >= 4) and (lon <= (pointsLon - 4)):
                    newa[int(timeInt), lat, lon] = (
                        0.5 * newa[int(timeInt) - 1, lat, lon + 3]
                        + 0.5 * newa[int(timeInt) + 1, lat, lon - 3]
                    )
        timeInt = timeInt + 2.0
    # ========================================================================================
    ...
    m = 0
    for lon in range(int(pointsLon)):
        if coordLon > (startLon + (n + 1) * stepLon) and coordLon < (
            startLon + (n + 2) * stepLon
        ):
            lowerIndexLon = n + 1
            higherIndexLon = n + 2
        n = n + 1
    for lat in range(int(pointsLat)):
        if coordLat < (startLat + (m + 1) * stepLat) and coordLat > (
            startLat + (m + 2) * stepLat
        ):
            lowerIndexLat = m + 1
            higherIndexLat = m + 2
        m = m + 1
    ...
    for m in range(totalmaps):
        RMSTECvalues.append(
            (1.0 - p) * (1.0 - q) * newa[m, lowerIndexLat, lowerIndexLon]
